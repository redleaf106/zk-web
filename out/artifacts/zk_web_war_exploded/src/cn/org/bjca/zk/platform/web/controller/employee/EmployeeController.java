...
public class EmployeeController extends BaseController{
	...
	public String list( ModelMap modelMap, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if(purpose==null||purpose.length()==0){
			...
			employeePage.setEmployeeName(employeeName);
			response.getWriter().write(JSONObject.toJSONString(employeePage.getData(), SerializerFeature.DisableCircularReferenceDetect));
			return null;
		}
	}
...
//		}
		employee.setIcCardNumber(employeeService.createEmployeeIcCardNumber());
...
//				}
	...
	public String insertOrUpdate(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		employeeExist.setIcCardNumber(employeeService.createEmployeeIcCardNumber());
...
//		}
    ...
    //安卓删除人员信息时后台需要删除人脸数据
	@RequestMapping("removeFaseData")
	@ResponseBody
	public String removeFaseData(String icCardNumber){
		System.out.println(icCardNumber);
		Employee employee = employeeService.findByicCardNumber(icCardNumber);
		if(employee!=null){
			employee.setPicFile(null);
			employeeService.saveOrUpdate(employee);
			if(employeeService.findByicCardNumber(icCardNumber).getPicFile()==null){
				return "success";
			}else {
				return "error";
			}
