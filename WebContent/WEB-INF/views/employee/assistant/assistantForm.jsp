...
$("#showPicId").click(function(){
	 if(checkAttachmentSize()){
	       ...
	       if (id != '') {
		       ...
		       $("#picId", navTab.getCurrentPanel()).attr('src', '${ctx}/employee/employee/showAssistantPic?id=' + id + "&" + Math.floor(Math.random() * 100));
	       } else {
...
<div class="pageContent">
	<form:form method="post" action="${ctx}/employee/employee/saveOrUpdateAssistant" enctype="multipart/form-data"  modelAttribute="assistant" class="pageForm required-validate" onsubmit="return iframeCallback(this, navTabAjaxDone)">
		<input type="hidden" name="id" value="${assistant.id}" />
		<div class="pageFormContent" layoutH="58">
			<div class="unit">
				<label>助理姓名：</label>
				 <input type="text" name="assistantName" size="40" minlength="2" maxlength="20" class="required" value="${assistant.assistantName}"/>
			</div>
			<div class="divider"></div>

			<div class="unit">
				<label>领导</label>
				<select name="leaderId" id="leaderId" class="required">
					<c:forEach var="employeeObj" items="${employeeList}" >
						 <option value="${employeeObj.id}" <c:if test="${employeeObj.id==assistant.leaderId}"> selected="selected"</c:if >> ${employeeObj.employeeName}</option>
					</c:forEach>
				</select>
			</div>
